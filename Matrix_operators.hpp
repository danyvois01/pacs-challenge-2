  ...
  // If the matrix is compressed
    ...
    if constexpr (Order == StorageOrder::RowMajor) {
      ...
      for (size_t i = inner_idx[row]; i < inner_idx[row + 1]; ++i) {
        //@note I think you have called outer the inner indexes... but is not
        //important
        //@note if the inner indexes (called outer_idx here) are sorted, you can
    ...
    throw std::runtime_error(
        "Cannot add new elements when the matrix is in compressed state.");
  }
}
...
const T Matrix<T, Order>::operator()(std::size_t row, std::size_t col) const {
  ...
  if (!is_compressed()) {
    ...
    // If the element does not exist, return a zero value
      ...
      return T(); //@note It works, but prefer T{} to T()
    }
  }
  // If the matrix is compressed
  else {
...
template <typename T, StorageOrder Order>
std::vector<T> operator*(const Matrix<T, Order> &mat,
                         const std::vector<T> &vec) {
  // Check if the dimensions of the matrix and vector are compatible for
  // multiplication
  if (mat.cols != vec.size()) {
    throw std::runtime_error("Error: Matrix and vector dimensions are not "
                             "compatible for matrix-vector multiplication.");
  }
